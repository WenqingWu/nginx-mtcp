    ...
    for (tries = 5; tries; tries--) {
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
            ...
            if (ls[i].inherited) {
                ...
                printf("---inherited socket, ignored.---\n");
                continue;
            }
			/*****************FLAG******************/
			if(ls[i].type != SOCK_STREAM)
				...
				s = (ngx_socket_t)mtcp_socket(mctx[cpu], AF_INET, SOCK_STREAM, 0);
            }
            if (s == (ngx_socket_t)-1) {
			...
			/*****************FLAG******************/
			if(ls[i].type == SOCK_STREAM) {
				if (mtcp_setsock_nonblock(mctx[cpu], s) < 0) {
					if (mtcp_close(mctx[cpu], s) == -1) {
	    				ngx_log_error(NGX_LOG_EMERG, log, ngx_socket_errno,
                ...
                == -1)
            {
				...
				/*****************FLAG******************/
//				if(ls[i].type == SOCK_STREAM)
//					ret = mtcp_close(mctx[cpu], s);
//				else 
					ret = ngx_close_socket(s);
...
//            printf("setting socket SO_REUSEADDR finished.\n");
#endif

#if (NGX_HAVE_REUSEPORT) /*与mTCP是独立的？*/
...
#endif
/*----------------mTCP only support ipv4 for the moment------------------*/
            ...
            if (!(ngx_event_flags & NGX_USE_IOCP_EVENT)) {
	            /*FLAG*/
                if (ls[i].type != SOCK_STREAM && ngx_nonblocking(s) == -1) {
  //              if (ngx_nonblocking(s) == -1) {
		...
		/***************FLAG*********************/
			if(ls[i].type != SOCK_STREAM)
				ret = bind(s, ls[i].sockaddr, ls[i].socklen);
			else {
                ret = mtcp_bind(mctx[cpu],s, ls[i].sockaddr, ls[i].socklen);
            }
...
//            if (listen(s, ls[i].backlog) == -1) {
			if ((mtcp_listen(mctx[cpu], s, 4096)) == -1) {
...
//                if (ngx_close_socket(s) == -1) {
            ...
            printf("---cpu = %d, listen socket created.---\n",cpu);
    ...
    if (failed) {
        printf("---open listening socket failed.---\n");
...
//            if (listen(ls[i].fd, ls[i].backlog) == -1) {
			if (mtcp_listen(mctx[cpu],ls[i].fd, ls[i].backlog) == -1) {
                printf("mtcp listen error.\n");
                ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
...
//        if (ngx_close_socket(ls[i].fd) == -1) {
		if (mtcp_close(mctx[cpu], ls[i].fd) == -1) {
...
//	if(c->type == SOCK_STREAM) {
		ret = mtcp_close(mctx[cpu], fd);
//	} else 
//		ret = ngx_close_socket(fd);
	if (ret == -1) {
        err = ngx_socket_errno;
