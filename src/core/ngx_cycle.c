...
//    n = old_cycle->listening.nelts ? old_cycle->listening.nelts : 10;
    ...
    for (i = 0; cycle->modules[i]; i++) {
		/*真正使用时只有ngx_core_module有create_conf回调函数*/
        if (cycle->modules[i]->type != NGX_CORE_MODULE) {
            continue;
        }

        module = cycle->modules[i]->ctx;
		/*会创建ngx_core_conf_t结构，用于存储整个配置文件main scope范围内的信息*/
		/*比如worker_processes，worker_cpu_affinity*/
	...
	/*初始化ngx_conf_t，用于解析配置文件并保存解析出来的信息*/
    ngx_memzero(&conf, sizeof(ngx_conf_t));
    /* STUB: init array ? */
	/*args是配置文件中指令的信息的数组，args[0]是指令名，args[1] - args[n]是指令的参数*/
    ...
    printf("---old listening socket, nelts: %ld.---\n", old_cycle->listening.nelts);
    if (old_cycle->listening.nelts) {
        ls = old_cycle->listening.elts;
        for (i = 0; i < old_cycle->listening.nelts; i++) {
            ls[i].remain = 0;
        }
		/*如果监听地址相同的话，则把新、旧cycle的监听socket合并*/
        ...
        for (n = 0; n < cycle->listening.nelts; n++) {
            ...
            for (i = 0; i < old_cycle->listening.nelts; i++) {
                ...
                if (ngx_cmp_sockaddr(nls[n].sockaddr, nls[n].socklen,
                    ...
                    == NGX_OK)
                {
                    printf("---same address, combine socket.---\n");
                    nls[n].fd = ls[i].fd;
                    nls[n].previous = &ls[i];
                    ls[i].remain = 1;
	...
	/*调用socket创建套接字 -> 调用setsockopt设置成可重用socket*/
	/*-> 设置成非阻塞socket -> 调用bind绑定要监听的socket地址 -> 调用listen转化成监听socket*/
//#if 0
    if (ngx_open_listening_sockets(cycle) != NGX_OK) {
        goto failed;
    }
	/* 根据cycle配置所有的监听socket */
    if (!ngx_test_config) {
