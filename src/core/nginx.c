...
/*****************FLAG******************/
static int get_num_cpu(const char *strnum);
...
/*----------------------FLAG--------------------------------*/
    ...
    mctx[cpu] = mtcp_create_context(cpu);
    if(!mctx[cpu]) {
        printf("cpu %d, create mtcp context failed.\n", cpu);
        return 1;
    }
...
#if 0
	for(j = 0; j < cpus; j++) {
		mctx[j] = mtcp_create_context(j);
		if(!mctx[j]) {
			ngx_log_stderr(0, "cpu%d create mTCP context failed.", j);
			return 1;
		}
	}
#endif
/*----------------------------------------------------------*/
	...
	/*-----------------FLAG----------------*/
	/*initialize mcfg.max_concurrency*/
	if(ccf->worker_processes)
		mcfg.max_concurrency = mcfg.max_num_buffers = cycle->connection_n * ccf->worker_processes * 0.9;
	else
		mcfg.max_concurrency = mcfg.max_num_buffers = cycle->connection_n;
#if !(NGX_WIN32)
...
/*-i-----------------------------FLAG-------------------------*/
    /*
     * destroy mTCP environment
     */
#if 0 
	for(j = 0; j < cpus; j++) {
		mtcp_destroy_context(mctx[j]);
	}
#endif
    ...
    for (i = 1; i < argc; i++) {
        ...
        while (*p) {
            ...
            case 'V':
                ...
                break;
/*----------------------  FLAG -------------------------*/
			/*n is consistent with worker_processes in the mtcp/nginx configuration file*/
			case 'n':
				if (*p) {
                    cpus = get_num_cpu((const char*) p);
                    if(cpus > core_limit){
                         printf("number of cores exceeds core limit.\n");
                         exit(0);
                     } 
                    goto next;
                }
                ...
                if (argv[++i]) {
                    cpus = get_num_cpu((const char *) argv[i]);
                    if(cpus > core_limit){
                        printf("number of cores exceeds core limit.\n");
                        return 0;
                    }
                ...
                ngx_log_stderr(0, "option \"-n\" requires number of cpu");
                return NGX_ERROR;
            /* multi process, the core_id on which the process should run */
            case 'r':
                flag = 1;
                if(*p) {
                    cpu = get_num_cpu((const char*) p);
                    if(cpu > core_limit - 1) {
                        printf("current cpu setting error.\n");
                        return NGX_ERROR;
                    }
                    goto next;
                }
                ...
                if(argv[++i]) {
                    cpu = get_num_cpu((const char*) argv[i]);
                    if(cpu > core_limit - 1) {
                        printf("current cpu setting error.\n");
                        return NGX_ERROR;
                    }
                ...
                ngx_log_stderr(0, "option \"-r\" requires current running cpu");
                return NGX_ERROR;
/*--------------------------------------------------------------*/
            case 't':
                ngx_test_config = 1;
